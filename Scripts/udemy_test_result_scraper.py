#!/usr/bin/env python
# This script intelligently converts a Udemy practice test results HTML file
# into a clean, structured Markdown file.
#
# usage: ./html_to_md.py <input1.html> <input2.html> ...
# example: ./html_to_md.py "results.html"
#
...
def process_and_clean_images(html_snippet_soup):
    ...
    # Find all the visible image wrappers and remove their hidden preceding twin
...
def format_as_markdown(data):
    ...
    if processed_explanation.strip():
        # --- CORRECTED QUOTE BLOCK SPACING ---
        # The '\n'.join() will handle the newlines perfectly this way.
        md_parts.append("> #### Overall explanation")
        md_parts.append(">")  # This creates the single empty line for spacing.
        for line in processed_explanation.split("\n"):
