...
# ==============================================================================
# 4. Asset Isolation & Injecting Precision Margins
# ==============================================================================
echo "Compiling using theme: $THEME_NAME"

TMP_CSS="$DIR_NAME/${THEME_NAME}.css"

# Copy the base theme CSS
cp "$THEME_CSS" "$TMP_CSS"

# Append explicit page geometry and an absolute scale reduction directly to the CSS
cat << 'EOF' >> "$TMP_CSS"

/* ==========================================================================
   Precision PDF Page Margins & Global Print Scale Overrides
   ========================================================================== */
@page {
    size: letter;
    ...
    margin-right: .75in !important;
}

/* 
   We target html and body cleanly without media blocks.
   This guarantees Chrome enforces it during script compilation.
*/
html, body {
    font-size: 12px !important; /* <--- Dropped down by 2 equivalent standard points */
}

/* Forces structural table content layouts to respect the downscale */
table, th, td {
    font-size: 0.85rem !important;
}

/* Keeps your terminal code fences sharp, compact, and perfectly scaled */
code, 
.md-fences,
.CodeMirror {
...
# ==============================================================================
echo "Step 1: Compiling HTML via Pandoc (Dropping standalone title block)..."
# Passing metadata.title="" forces Pandoc to omit its native top header container
pandoc "$INPUT_MD" \
  --standalone \
  --metadata title="" \
